...
WireGuard \cite{b3} is a modern VPN protocol. It is designed to be simple, fast, and secure. Traditional VPN solutions like IPsec work at Layer 3. OpenVPN requires complex handshaking. WireGuard uses a simpler approach. It uses Curve25519 for key exchange, ChaCha20 for encryption, and Poly1305 for authentication. The code is small (about 4,000 lines). This makes it easy to check and maintain.
...
\subsection{NAT Types and Behavior}

Network Address Translation comes in several types, each with different behavior for inbound connections \cite{b4}. Understanding these differences is important for peer-to-peer communication:

\textbf{Full-Cone NAT}: Once an internal address (IP:port) is mapped to an external address (IP:port), any external host can send packets to the internal host by sending to the mapped external address. This is the most permissive type and works well with peer-to-peer connections.

\textbf{Restricted-Cone NAT}: The NAT only allows inbound packets from external hosts that the internal host has previously sent packets to. However, the external host can send packets back from any source port. This means both peers must send packets to establish bidirectional communication, but the port number is not restricted.

\textbf{Port-Restricted-Cone NAT}: Similar to Restricted-Cone NAT, but the NAT also checks the source port number. The external host must send from the same port number that received packets from the internal host. This is the most common NAT type in home routers.

\textbf{Symmetric NAT}: The NAT creates a different mapping for each destination address and port. When the internal host sends to different external destinations, the NAT uses different external ports for each connection. This makes peer-to-peer connections very difficult because the external port seen by a STUN server differs from the port seen by the actual peer.
...
\textbf{Tailscale} \cite{b6} embeds a modified version of wireguard-go (the Go userspace implementation of WireGuard) in its application. It modifies wireguard-go to integrate STUN-based NAT traversal and DERP (Designated Encrypted Relay for Packets) relay servers when direct connections fail. Tailscale is easy to use and works well with NAT. However, it requires coordination servers (either Tailscale's hosted service or self-hosted Headscale) and cannot use kernel WireGuard modules directly.

\textbf{Netmaker} \cite{b7} focuses on mesh networking and uses a client-server architecture. The Netclient agent configures kernel WireGuard on managed nodes. However, Netmaker requires a coordination server with a public static IP address and open firewall ports for node communication. This adds deployment complexity and infrastructure requirements compared to solutions that only need access to public STUN servers.

\textbf{wireguard-p2p} \cite{b8} by Manuel Schölling inspired STUNMESH-go's design. It uses STUN for NAT traversal and OpenDHT for endpoint exchange. However, it did not have enough flexibility for production use. It had limitations with storage backends and multi-peer scenarios.
...
\textbf{Establish Controller} gets peer endpoints from storage plugins and decrypts them. It configures WireGuard peer settings using wgctrl library calls. This controller handles the WireGuard API interaction. It updates peer endpoints when they change.

\textbf{Refresh Controller} triggers periodic updates by requesting the Publish Controller to re-discover endpoints and the Establish Controller to refresh peer configurations. It runs at a configured interval. This ensures endpoints stay current as network conditions change.
...
The Linux STUN implementation is the most advanced part of STUNMESH-go. It creates raw IP sockets (using \texttt{net.ListenPacket("ip4:17", "0.0.0.0")} for IPv4). These bypass the kernel's UDP socket handling. This allows the application to build complete UDP packets including headers. Berkeley Packet Filter (BPF) \cite{b9} programs filter incoming traffic. They capture only STUN responses for the WireGuard port.
...
The wgctrl library \cite{b10} simplifies WireGuard userspace API access across platforms. However, platforms behave differently when updating peer configurations. Linux WireGuard allows updating existing peer endpoints without removing and re-adding peers (UpdateOnly mode). This provides atomic updates without disrupting the connection state.

FreeBSD's kernel WireGuard implementation does not yet support the UpdateOnly feature. The wgctrl-go library returns \texttt{ErrUpdateOnlyNotSupported} when this flag is set on FreeBSD. The STUNMESH-go code sets \texttt{UpdateOnly = false} for FreeBSD. This allows the library to remove and re-add peers if necessary:
...
\item Captures STUN responses on the right interface
...
\begin{itemize}
\item Full-Cone NAT (home routers): Bidirectional connectivity established
\item Port-Restricted-Cone NAT (enterprise firewalls): Connectivity established after initial handshake
\item CGNAT (mobile carriers): Connectivity achieved when at least one peer uses cone NAT
\end{itemize}
...
\textbf{BSD ICMP Health Checks}: Without SO\_BINDTODEVICE support, BSD platforms rely on routing tables to direct ICMP ping traffic through WireGuard tunnels. This works well in typical single-WAN scenarios but may require additional routing configuration in complex multi-path setups.
...
The complete code is available under the GPLv2 or later license at \url{https://github.com/tjjh89017/stunmesh-go}. We welcome contributions from the BSD and broader networking community.
...
\bibitem{b1} pfSense Project, ``pfSense Documentation,'' \url{https://docs.netgate.com/pfsense/}, 2025.
\bibitem{b2} OPNsense Project, ``OPNsense Documentation,'' \url{https://docs.opnsense.org/}, 2025.
\bibitem{b3} J. Donenfeld, ``WireGuard: Next Generation Kernel Network Tunnel,'' in Proceedings of NDSS 2017, 2017.
\bibitem{b4} F. Audet and C. Jennings, ``Network Address Translation (NAT) Behavioral Requirements for Unicast UDP,'' RFC 4787, January 2007.
\bibitem{b5} J. Rosenberg et al., ``Session Traversal Utilities for NAT (STUN),'' RFC 5389, October 2008.
\bibitem{b6} Tailscale Inc., ``How Tailscale Works,'' \url{https://tailscale.com/blog/how-tailscale-works/}, 2023.
\bibitem{b7} Gravitl, ``Netmaker: WireGuard Automation Platform,'' \url{https://github.com/gravitl/netmaker}, 2025.
\bibitem{b8} M. Schölling, ``wireguard-p2p: A tool for setting up WireGuard connections from peer to peer,'' \url{https://github.com/manuels/wireguard-p2p}, 2017.
\bibitem{b9} S. McCanne and V. Jacobson, ``The BSD Packet Filter: A New Architecture for User-level Packet Capture,'' in Proceedings of the USENIX Winter 1993 Conference, 1993.
\bibitem{b10} WireGuard Project, ``wgctrl-go: Package wgctrl enables control of WireGuard interfaces on multiple platforms,'' \url{https://github.com/WireGuard/wgctrl-go}, 2025.
\end{thebibliography}
